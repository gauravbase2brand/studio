...
export default function EditOfferPage() {
  ...
  useEffect(() => {
    if (id) {
        ...
        const currentOffer = storedOffers.find(o => o.id === id);
        if (currentOffer) {
            setOffer(currentOffer);
            setTitle(currentOffer.title);
            setCode(currentOffer.code);
            setDescription(currentOffer.description);
            setDiscountType(currentOffer.discountType);
            setDiscountValue(String(currentOffer.discountValue));
            setStartDate(currentOffer.startDate);
            setEndDate(currentOffer.endDate);
            setFoodItemIds(currentOffer.foodItemIds || (currentOffer.foodItemId ? [currentOffer.foodItemId] : []));
        } else {
  ...
  const handleUpdateOffer = () => {
    ...
    // remove obsolete foodItemId if it exists
    delete updatedOffer.foodItemId;

    try {
      const existingOffers = JSON.parse(localStorage.getItem('offers') || '[]');
      const updatedOffers = existingOffers.map(o => o.id === id ? updatedOffer : o);
      localStorage.setItem('offers', JSON.stringify(updatedOffers));
      window.dispatchEvent(new Event('storage'));

      toast({
        title: "Offer Updated Successfully!",
        description: `The offer "${title}" has been updated.`,
      });
      router.push('/dashboard/offers');
    } catch (error) {
       toast({
        title: "Failed to Update Offer",
        description: "There was an error saving the offer.",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <>
      <DashboardHeader title="Edit Offer">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Update the details for this promotional offer.</CardDescription>
        </CardHeader>
                        ...
                        <ScrollArea className="h-72">
                            <div className="p-2">
                                {dishes.map((dish) => (
                                    ...
                                    >
                                        <Checkbox id={`dish-checkbox-edit-${dish.id}`} checked={foodItemIds.includes(dish.id)} />
                                        <Label htmlFor={`dish-checkbox-edit-${dish.id}`} className="font-normal flex-1 flex items-center gap-2 cursor-pointer text-left">
